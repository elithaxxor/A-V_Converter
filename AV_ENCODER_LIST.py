...
class VideoEditor():
    ...
    def textLen0(self):
        textlength = self.noOfClasses
        print(f'{yellow}[+] Reading [{textList}] items'); time.sleep(1)
        return textlength

    def readList(self):
        #self.streamVideo = ffmpeg.input(self.vid_loc)
        global curImg, current_image_path
        ...
        try:
            for x in range(0, (self.noOfClasses - 1)):
                #   file_list = os.mkdir(path+"/"+str(x)) ## change
                myPicList = os.listdir(self.path)  ## store in list
                print('[+] List of Item in Dir:\n ', myPicList)
                class_count += 1
                for pics in myPicList:
                    pic = myPicList[class_count]
                    print(f'\nProcessing : [{pic}] ')
                    current_image_path = self.path + '/' + pic
                    curImg = ffmpeg.input(current_image_path)
                    self.streamVideo = ffmpeg.input(current_image_path)
                    AV00 = np.array(AV00)
...
def main():
    ...
    print(f'{yellow}[+] A/V Converter.{reset}')

    try:
        print(f'{yellow}[+] Read ALL AV from dir?{reset}')
        readListq = input('**')
    ...
    try:
        print(f"\n {'X' * 50}")  ## ENCODER
        print(f'{yellow}[+] Change Encoding? {reset}')
        encodeA = input('')
        ...
        print(f"\n {'X' * 50}")  ## TRIMMER
        print(f'{yellow}[+] Trim Video?.{reset}')
        trimA = input('')
        ...
        print(f'{yellow}[+] Change FPS? {reset}')  ## FPS
        fpsA = input('')
        ...
        ## VIDEO SCALER
        print(f"\n {'X' * 50}")  ## TRIMMER
        print(f'{yellow}[+] Scale Video?.{reset}')
        scaleA = input('')
